...
const MainPage = () => {
  ...
  const handleLike = async (e, id) => {
    ...
    try {
      ...
      M.toast({html: 'Ошибка лайка', classes: 'red'})
    }
  }
  ...
  const createInventory = async () => {
    if (!title.trim() || !categoryId) {
      return M.toast({html: 'Заполните название и категорию', classes: 'red'})
    }
    try {
      ...
      setCustomFields([])
      fetchData()
      M.toast({html: 'Коллекция создана', classes: 'green'})
    } catch (e) {
      M.toast({html: 'Ошибка при создании', classes: 'red'})
    }
  }
      ...
      <div className="main-page">
        ...
        <div className="row card-panel">
          <div className="input-field col s12 m6">
            ...
            <label htmlFor="title" className="active">Название коллекции</label>
          </div>
          ...
          <div className="input-field col s12">
            ...
            <label htmlFor="desc" className="active">Описание</label>
          </div>

          <div className="input-field col s12">
            <input id="tags" type="text" value={tags} onChange={e => setTags(e.target.value)} placeholder="книги, редкое, 2024" />
            <label htmlFor="tags" className="active">Теги (через запятую)</label>
          </div>

          <div className="col s12">
            <h6>Дополнительные поля предметов:</h6>
          ...
          <div className="col s12 m6 right-align" style={{marginTop: '20px'}}>
            <button className="btn-large blue waves-effect waves-light" onClick={createInventory}>Создать</button>
          </div>
        </div>

        <div className="row valign-wrapper" style={{marginTop: '40px', marginBottom: '10px'}}>
          <div className="col s8">
            <h3 style={{margin: 0}}>Мои коллекции</h3>
          </div>
        ...
        <div className="collection z-depth-1">
          ...
          {filteredInventories.length === 0 && (
            <div className="collection-item center-align grey-text" style={{padding: '30px'}}>
              ...
              {searchQuery || selectedTag ? 'Ничего не найдено' : 'Коллекций пока нет'}
            </div>
          )}
        </div>
